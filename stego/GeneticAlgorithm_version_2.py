...
class GeneticAlgorithm:
    def __init__(self, n_pop, pc, pm, epoch, can_loc_HH, can_loc_HL, can_loc_LH, cover_k, LL, LH, HL, HH,  HHprim, HLprim, LHprim, data_k_HH, data_k_HL, data_k_LH, mul, key, HH_keys, HL_keys, LH_keys, eng):
        """Initializes the genetic algorithm with all necessary parameters and state variables."""
        self.n_pop = n_pop
        self.pc = pc
        self.pm = pm
        self.epoch = epoch
    ...
    def findBestKey(self):
        """Executes the genetic algorithm to find the best seeds for data embedding."""
    ...
    def selection(self, pop_dict):
        """Selects the top individuals based on fitness scores."""
    ...
    def mutation(self):
        """Performs mutation operations on the population to introduce genetic diversity."""
    ...
    def cross_over(self):
        num_cross = int((self.pc * self.n_pop)/2)
        pop_size = self.pop.size
    ...
    def fitness(self, chromo):
        """Calculates the fitness of an individual by embedding data and evaluating image quality."""
        HHS = embedding(self.HH.copy(), self.HHprim, self.can_loc_HH, chromo, self.data_k_HH, self.mul, self.HH_keys)
        HLS = self.HL
        LHS = self.LH
        if self.HLprim is not None:
            HLS = embedding(self.HL.copy(), self.HLprim, self.can_loc_HL, chromo, self.data_k_HL, self.mul, self.HL_keys)
        if self.LHprim is not None:
            LHS = embedding(self.LH.copy(), self.LHprim, self.can_loc_LH, chromo, self.data_k_LH, self.mul, self.LH_keys)
    ...
    def picture_fitness(self, cover, stego_image):
        """Computes the fitness PSNR score between the cover and stego images."""
        m, n = cover.shape
        suma = 0
